...
#include <iostream>
#include <vector>
#include "../day9-10/stb/stb_image.h"
#include "../day9-10/stb/stb_image_write.h"
...
public:
    ...
    std::vector<std::vector<RGB>> createCanvas(int canvasHeight, int canvasWidth)
    {
        std::vector<std::vector<RGB>> canvas(canvasHeight, std::vector<RGB>(canvasWidth, {255, 255, 255}));
        width = canvasWidth;
        height = canvasHeight;
        return canvas;
    }

    void placeTileOnCanvas(std::vector<std::vector<RGB>> &canvas, const std::vector<std::vector<RGB>> &image, int x, int y)
    {
        ...
        for (int i = 0; i < imageHeight; ++i)
        {
            for (int j = 0; j < imageWidth; ++j)
            {
                ...
                // Ensure we're within the bounds of the canvas
                if (canvasY >= 0 && canvasY < canvasHeight && canvasX >= 0 && canvasX < canvasWidth)
                {
                    canvas[canvasY][canvasX] = image[i][j];
                }
            }
        }
    };
